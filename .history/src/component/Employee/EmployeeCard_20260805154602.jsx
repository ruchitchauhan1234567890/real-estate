...
const EmployeeCard = ({ data, setData }) => {
    ...
    const totalEmployee = employee.filter((curr) => {
        return !curr.isAdmin
    }).length

     const inactiveEmp = employee.filter((curr) => {
        if (curr.isAdmin) return
        return curr.status === "Inactive"
    }).length

    const activeEmp = employee.filter((curr) => {
        if(curr.isAdmin) return
        ...
        {cards.map((data, index) => {
            return (
                <div key={index} className="w-full h-15 shadow-sm rounded-sm px-2  flex items-center">
                    <div className="w-auto h-auto items-center bg-[#E8DADA] rounded-sm p-1 mr-4 m-2">
