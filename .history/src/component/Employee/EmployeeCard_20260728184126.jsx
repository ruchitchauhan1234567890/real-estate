...
const EmployeeCard = ({ data, setData }) => {
    ...
    const totalEmployee = employee.filter((curr) => curr.status === "Active").length
    const activeEmp = employee.filter((curr) => curr.status === "Inactive").length
    ...
    return (
        <div className="grid grid-cols-3 gap-3 mt-3  place-items-center">
            {cards.map((data,index) => {
                return (
                    <div key={index} className="w-full h-15 shadow-sm rounded-sm flex items-center">
