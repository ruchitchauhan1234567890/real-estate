  ...
  return (
    <div className="flex justify-between items-center shadow-sm h-12 px-2">
      ...
      <div>
        ...
        { loggedUser.isAdmin && <button className="px-2 py-1 m-2 rounded-sm bg-blue-400" onClick={() => setOpen(!open)}>Add Lead</button>}
      </div>
      { loggedUser.isAdmin && open && <AddLead open={open} setOpen={setOpen} leadData={leadData} setLeadData={setLeadData} />}
