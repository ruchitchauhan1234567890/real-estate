...
const LeadsForm = ({ leadData, setLeadData, selectedLead }) => {
  ...
  return (
    <div className="flex justify-between items-center shadow-sm h-12 px-2">
      ...
      <div>
        <button disabled={selectedLead.length == 0} className={`px-2 py-1 m-2 rounded-sm ${selectedLead.length == 0 ? "bg-gray-400 cursor-not-allowed" : "bg-blue-400 cursor-pointer "}`}></button>
