...
const LeadsForm = ({ leadData, setLeadData, selectedLead }) => {
  ...
  return (
    <div className="flex justify-between items-center shadow-sm h-12 px-2">
      ...
      <div>
        <button disabled={selectedLead.length == 0} className={` ${selectedLead.length == 0 ? " px-2 w-20 h-auto py-1 m-2 rounded-sm bg-gray-400 cursor-not-allowed" : " px-2 py-1 m-2 rounded-sm  bg-blue-400 cursor-pointer "}`}></button>
