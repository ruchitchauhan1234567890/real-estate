...
export const monthlyConversionRate = (lead = []) => {
    ...
    const data = lead.reduce((acc, curr) => {
        ...
        // Count all leads of this month
        acc[month].totalLeads++

        // Count converted leads of this month
    ...
    return Object.values(data).map((item) => ({
        ...
        month: new Date(item.month + "-01").toLocaleDateString(
            "en-US",
            {
                month: "short",
                year: "numeric"
        ...
        conversionRate:
            item.totalLeads > 0
                ? Number(
                    (
                        (item.convertedLeads / item.totalLeads) * 100
                    ).toFixed(2)
                )
                : 0
    }))
