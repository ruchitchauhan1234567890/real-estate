...
const TaskTable = () => {
    ...
    return (
        <div>
            <div className="flex gap-2 mt-2">
                <div>
                    <input type="text"
                        ...
                        className="w-72 rounded-sm border border-gray-300 px-4 py-[6px] outline-none focus:ring-2 focus:ring-blue-500"
                        onChange={handleChange} />
                </div>
                <div className="">
                    {/* <select onChange={handleChange}className="rounded-sm border border-gray-300 px-4 py-2 focus:ring-2 focus:ring-blue-500" name="priority" value={filters.priority}>
                        <option>All</option>
                        <option>High</option>
                     ...
                     <Select
                        ...
                        placeholder={filters.priority}
                        value={filters.priority}
                        onChange={(select) => {
                            setFilters((prev) => ({ ...prev, priority: select.value }))
                        }}
                        className="w-30"
                    />
                </div>
                <div className="">
                    {/* <select onChange={handleChange} className="rounded-sm border border-gray-300 px-4 py-2 focus:ring-2 focus:ring-blue-500" name="taskStatus" value={filters.taskStatus}>
                        <option>All</option>
                     ...
                     <Select
                        ...
                        placeholder={filters.taskStatus}
                        // value={filters.taskStatus}
                        onChange={(select) => {
                            setFilters((prev) => ({ ...prev, taskStatus: select.value }))
                        }}
                        // className="rounded-sm border border-gray-300 px-4 py-2 focus:ring-2 focus:ring-blue-500"
                        className="w-40 "
                    />
                </div>
                <button className="rounded-sm border border-gray-300 bg-gray-200 px-4 py-[5px] focus:ring-2 focus:ring-blue-500" onClick={() => setFilters({
                    taskStatus: "All",
                    priority: "All",
                    searchTask: ""
                })}>Reset Filter</button>
            ...
            <div className="overflow-hidden rounded-sm border border-gray-300 shadow-sm mt-2">
