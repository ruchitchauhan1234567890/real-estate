const TodaysFollowUp = ({ data }) => {
    ...
    const todays = data?.filter((item) => item.nextFollowUpDate === formattedDate).slice(0,3) || [];

    return (
        <div className="w-full h-full shadow-sm rounded-2xl  overflow-y-auto">
            <p className="font-bold mt-4 px-3">Todays Follow-up</p>
            {todays.map((item) => (
